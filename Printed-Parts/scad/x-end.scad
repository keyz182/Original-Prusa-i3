...
module x_end_holes(){
    ...
    // TR Nut trap
    // Hole for the nut
    translate(v=[0,-17, -1]) poly_cylinder(h = 9.01, r = 5.3, $fn = 25);
    

    // Screw holes for TR nut
    translate(v=[0,-17, 0]) rotate([0, 0, -135]) translate([0, 8.2, -1]) cylinder(h = 10, r = 1.55, $fn=25);
    translate(v=[0,-17, 0]) rotate([0, 0, -135]) translate([0, -8.2, -1]) cylinder(h = 10, r = 1.55, $fn=25);

    // Nut traps for TR nut screws
    translate(v=[0,-17, 0]) rotate([0, 0, -135]) translate([0, 8.2, 6]) rotate([0, 0, 0])cylinder(h = 3, r = 3.3, $fn=6);

    translate(v=[0,-17, 0]) rotate([0, 0, -135]) translate([0, -8.2, 6]) rotate([0, 0, 30])cylinder(h = 3, r = 3.2, $fn=6);
    translate([-5.5,-17.2,6]) rotate([0,0,30]) cube([5,5,3]);
    translate([-0,-17.2,6]) rotate([0,0,60]) cube([5,10,3]);
    
    translate(v=[0,-17, 7.8]) poly_cylinder(h = 60, r = 7, $fn = 25);
}
